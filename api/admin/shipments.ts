...
async function listShipments(client: Client, number?: string) {
  ...
  const { rows } = await client.query(
    `SELECT s.tracking_number, s.user_id, s.service_id, s.status, s.estimated_delivery,
            ...
            EXISTS (
              SELECT 1 FROM tracking_images i
              WHERE i.tracking_number = s.tracking_number AND i.event_type = 'setup'
            ) AS has_setup_image,
            EXISTS (
              SELECT 1 FROM tracking_images i
              WHERE i.tracking_number = s.tracking_number AND i.event_type = 'delivered'
            ) AS has_delivered_image
     FROM shipments s
     ${where}
  ...
  if (rows.length) {
    ...
    const ev = await client.query(
      `SELECT tracking_number, event_time, location, status, details
...
export default async function handler(req: any, res: any) {
  ...
  if ((req.method !== 'GET' || listingAll) && !isAdmin(req)) {
  ...
  try {
    if (req.method === 'GET') {
      ...
      const data = await withDb((c) => listShipments(c, number));
      ...
      await withDb(async (c) => {
        await c.query(
          `INSERT INTO shipments (
             tracking_number, status, origin, destination, service, service_id,
             current_location, estimated_delivery_text, updated_at
        ...
        await c.query(
          `INSERT INTO shipment_events (tracking_number, location, status, details)
           VALUES ($1, $2, $3, $4)`,
          [number, b.location || b.destination || '', b.status || 'In transit', 'Admin update']
        );

        await c.query('INSERT INTO admin_activity (action, detail) VALUES ($1, $2)', [
          'Shipment saved',
          `${number} -> ${b.status || 'In transit'}`,
        ]);
      });
