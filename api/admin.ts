...
async function withDb(fn: (client: any) => Promise<any>) {
  ...
  try {
    ...
    await client.query('ALTER TABLE shipments ADD COLUMN IF NOT EXISTS hold_reason TEXT');
    await client.query('ALTER TABLE shipments ADD COLUMN IF NOT EXISTS deleted_at TIMESTAMPTZ');
  ...
  try {
      ...
      const shipments = await withDb(async (c) => {
        ...
        const { rows } = await c.query(
          `SELECT s.tracking_number, s.status, s.origin, s.destination, s.service, s.service_id,
                  ...
                  s.fee_paid, s.collect_payment, s.payment_instructions, s.notify_email, s.notify_enabled, s.hold_reason, s.hold_location,
           ...
           FROM shipments s ${where} ${where ? 'AND' : 'WHERE'} (s.deleted_at IS NULL) ORDER BY s.updated_at DESC NULLS LAST`, params);
    ...
    if (req.method === 'DELETE') {
      ...
      await withDb(async (c) => {
        ...
        try { await c.query('ALTER TABLE shipments ADD COLUMN IF NOT EXISTS deleted_at TIMESTAMPTZ'); } catch { /* ok */ }
        await c.query('UPDATE shipments SET deleted_at = now(), updated_at = now() WHERE tracking_number = $1', [number]);
    ...
    if (req.method === 'POST') {
      ...
      const holdReasonEarly = String(body.holdReason || body.hold_reason || '').trim();
      const details = publicEventDetails(
        status,
        holdReasonEarly
          ? [holdReasonEarly, body.details || body.message].filter(Boolean).join(' · ')
      ...
      await withDb(async (c) => {
        ...
        const holdReason = String(body.holdReason || body.hold_reason || '').trim();
        if (holdReason || /hold|exception/i.test(String(status))) {
          try { await c.query('ALTER TABLE shipments ADD COLUMN IF NOT EXISTS hold_reason TEXT'); } catch { /* ok */ }
          await c.query(
            `UPDATE shipments SET hold_reason = $2, hold_location = COALESCE(NULLIF($3, ''), hold_location, current_location)
             WHERE lower(tracking_number) = lower($1)`,
            [number, holdReason || null, location || '']
          );
        }
        await c.query('INSERT INTO admin_activity (action, detail) VALUES ($1, $2)', [
          'Shipment saved',
