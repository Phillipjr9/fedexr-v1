...
function stageFromStatus(status: string): number {
  ...
  if (/exception|held|delay|customs|returned/.test(s)) return 2;
...
function stageFromEvents(events: TrackingEvent[]): number {
  let max = 0;
  for (const ev of events || []) {
    ...
    else if (/exception|held|delay|customs|returned/.test(s)) max = Math.max(max, 2);
...
function eventForStage(events: TrackingEvent[], stageTarget: number): TrackingEvent | null {
  ...
  for (const ev of list) {
    ...
    else if (/exception|held|delay|customs|returned|transit|facility|on the way|departed|arrived|en route|hub|in transit/.test(s)) st = 2;
...
export default function TrackingPage() {
  ...
  const stage = result
    ? Math.max(
        stageFromStatus(result.status),
        stageFromEvents(result.events || [])
      )
    : 0;
    ...
    <div className="min-h-screen bg-[#F7F7F8] text-gray-900 track-print-root">
      <style>{`
        @media print {
          ...
          .progress-dot-pulse, .progress-ring-pulse, .progress-line-flow { animation: none !important; }
        }
        @keyframes progress-pulse {
          0%, 100% { transform: scale(1); opacity: 0.35; }
          50% { transform: scale(1.45); opacity: 0; }
        }
        @keyframes progress-soft-glow {
          0%, 100% { box-shadow: 0 0 0 0 rgba(77, 20, 140, 0.35); }
          50% { box-shadow: 0 0 0 10px rgba(77, 20, 140, 0); }
        }
        @keyframes progress-soft-glow-green {
        ...
        .progress-ring-pulse {
          ...
          background: rgba(77, 20, 140, 0.25);
          animation: progress-pulse 2s ease-out infinite;
          pointer-events: none;
        }
        .progress-ring-pulse-green { background: rgba(22, 163, 74, 0.25); }
        .progress-dot-active { animation: progress-soft-glow 2s ease-in-out infinite; }
        .progress-dot-active-green { animation: progress-soft-glow-green 2s ease-in-out infinite; }
        .progress-dot-done { animation: progress-dot-pop 0.45s cubic-bezier(0.34, 1.4, 0.64, 1) both; }
        .progress-line-flow {
          background-image: linear-gradient(180deg, rgba(77, 20, 140, 1) 0%, rgba(77, 20, 140, 0.55) 40%, rgba(77, 20, 140, 1) 80%, rgba(77, 20, 140, 0.7) 100%);
          background-size: 100% 200%;
          animation: progress-line-flow 2.8s linear infinite;
        }
        .progress-line-flow-green {
          background-image: linear-gradient(180deg, rgba(22, 163, 74, 1) 0%, rgba(22, 163, 74, 0.55) 40%, rgba(22, 163, 74, 1) 80%, rgba(22, 163, 74, 0.7) 100%);
        }
        @media (prefers-reduced-motion: reduce) {
          .progress-ring-pulse, .progress-dot-active, .progress-dot-active-green, .progress-dot-done, .progress-line-flow { animation: none !important; }
        }
      `}</style>
            ...
            <div className="mx-4 mt-3 rounded-xl border border-amber-200/80 bg-white shadow-sm overflow-hidden print-break no-print">
              <button type="button" onClick={() => setPayOpen((v) => !v)} className="w-full flex items-center gap-3 px-4 py-3 text-left">
                ...
                <div className="min-w-0 flex-1">
                  ...
                  <p className="text-xs text-gray-500">Pay offline to complete shipping charges</p>
                </div>
            ...
            <div className="relative pl-11">
              <div
                className={`absolute left-[15px] top-4 bottom-6 w-[3px] rounded-full ${
                  delivered
                    ? 'bg-green-600 progress-line-flow progress-line-flow-green'
                    : stage > 0
                      ? 'progress-line-flow'
                      : 'bg-gray-200'
                }`}
...
function ProgressStep({
  ...
  const glowClass = delivered && isActive
  ...
  return (
    <div className={`relative ${last ? 'pb-1' : 'pb-8'}`}>
      <div className="absolute -left-11 top-0 flex h-9 w-9 items-center justify-center z-10">
        {isActive && (
          <span
            className={`progress-ring-pulse ${delivered ? 'progress-ring-pulse-green' : ''}`}
            aria-hidden
          />
        )}
        {icon === 'from' && (isDone || isActive) ? (
          <span className={`relative flex h-8 w-8 items-center justify-center rounded-full ${doneColor} shadow-sm ${glowClass}`}>
            <MapPin className="h-4 w-4 text-white" strokeWidth={2.5} />
          </span>
        ) : icon === 'transit' && isActive ? (
          <span className={`relative flex h-9 w-9 items-center justify-center rounded-full bg-[#4D148C] shadow-md ring-4 ring-[#4D148C]/20 ${glowClass}`}>
            <Navigation className="h-4 w-4 text-white" strokeWidth={2.5} />
          </span>
        ) : icon === 'done' && isDone ? (
          <span className={`relative flex h-9 w-9 items-center justify-center rounded-full bg-green-600 shadow-md ring-4 ring-green-600/20 ${glowClass}`}>
          ...
          <span className={`relative h-3.5 w-3.5 rounded-full ${doneColor} ring-4 ring-white ${glowClass}`} />
        ) : (
          <span className="relative h-3.5 w-3.5 rounded-full bg-gray-300 ring-4 ring-white transition-colors duration-300" />
        )}
      </div>
      <div
        className={`rounded-xl px-3 py-2 -ml-1 transition-all duration-300 ${
          isActive ? 'bg-gray-50 ring-1 ring-gray-100 shadow-sm' : ''
        }`}
      >
        <p className={`text-[11px] font-bold uppercase tracking-wide transition-colors duration-300 ${
          isTodo ? 'text-gray-400' : delivered && isDone && last ? 'text-green-700' : isActive ? 'text-[#4D148C]' : 'text-gray-900'
        }`}>{title}</p>
        {primary ? (
          <p className={`text-[15px] mt-0.5 font-semibold transition-colors duration-300 ${isTodo ? 'text-gray-400' : delivered && last ? 'text-green-700' : 'text-gray-900'}`}>{primary}</p>
        ) : null}
        {secondary ? (
          <p className={`text-sm mt-0.5 transition-colors duration-300 ${isTodo ? 'text-gray-400' : 'text-gray-600'}`}>{secondary}</p>
        ) : null}
        {tertiary ? (
          <p className={`text-sm mt-0.5 transition-colors duration-300 ${isTodo ? 'text-gray-400' : 'text-gray-500'}`}>{tertiary}</p>
        ) : null}
      </div>
    </div>
  );
