...
export type ImageEventType = 'setup' | 'transit' | 'delivered' | 'hold';
...
export async function apiListImages(trackingNumber: string) {
  const res = await fetch(
    `/api/get-tracking-image?number=${encodeURIComponent(trackingNumber)}&list=1`
  );
  if (!res.ok) return { images: [] as { id: number; eventType: string; dataUrl: string }[] };
  const json = await res.json().catch(() => ({}));
  const images = Array.isArray(json.images) ? json.images : [];
  return {
    images: images.map((i: any) => ({
      id: i.id,
      eventType: i.eventType || i.event_type || 'setup',
      dataUrl: i.dataUrl,
    })),
  };
}
...
export async function apiGetBanner() {
  ...
  const json = await res.json().catch(() => ({}));
  return json.banner as { enabled: boolean; message: string; linkText: string; linkHref: string } | null;
}

export async function apiSaveBanner(body: {
  enabled: boolean;
  message: string;
  linkText: string;
  linkHref: string;
